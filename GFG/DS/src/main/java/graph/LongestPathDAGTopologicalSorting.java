...
public class LongestPathDAGTopologicalSorting {
		...
		static void topologicalSort(Graph g, Stack<Integer> s, boolean[] visited, int i) {
			...
			Iterator<graph.LongestPathDAGTopologicalSorting.Graph.Node> iterator = g.adjList[i].iterator();

			while (iterator.hasNext()) {
				graph.LongestPathDAGTopologicalSorting.Graph.Node next = iterator.next();
		...
		static void longestPath(Graph g, int src) {
			...
			while (!s.isEmpty()) {
				...
				if (dist[u] != Integer.MIN_VALUE) {
					Iterator<graph.LongestPathDAGTopologicalSorting.Graph.Node> iterator = g.adjList[u].iterator();
					while (iterator.hasNext()) {
						graph.LongestPathDAGTopologicalSorting.Graph.Node next = iterator.next();
						int v = next.data;
