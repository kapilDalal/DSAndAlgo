package challenge.weekly.October25;
...
subarray nums[l[i]], nums[l[i]+1], ... , nums[r[i]] can be rearranged to form an arithmetic sequence,
and false otherwise.
...
-105 <= nums[i] <= 105
* */
public class AirthmeticSubarray {
    ...
    public List<Boolean> checkArithmeticSubarrays(int[] nums, int[] l, int[] r) {
        ...
        for(int i=0;i<l.length;i++){
            ...
            ArrayList<Integer> asc = new ArrayList<>(sub);
            ArrayList<Integer> dsc = new ArrayList<>(sub);
            Collections.sort(asc);
            Collections.sort(dsc,Collections.reverseOrder());
            if(present(asc) || present(dsc)){
                res.add(true);
            }else{
                res.add(false);
            }
    ...
    private static boolean present(ArrayList<Integer> l) {
        if(l.size()<=2)
            return true;
